...
class Math:
    ...
    def graph_status(self, sequence: list[Numeric]) -> str:
        info: list[Numeric] = sequence[-10:-2]
...
class LogicLink:
    ...
    async def run(self, data: str | dict[str, str | float | dict] | Ticker) -> None:
        ...
        if (db_len := len(price_db)) > 300:
            ...
            if status == "fall":
                checking_point_index: int = last_point_index if last_point_index - 150 > highest_point_index else highest_point_index
                checking_point_index = price_db.index(max(price_db[highest_point_index -150:]))
                # TRACKER
            ...
            if self.transactions:
                for transaction in self.transactions:
                    ...
                    if status == "rise" and difference_in_precent >= 0.4:
                        ...
                        if transaction_data.get("counter") >= 5:
                            self.loop.create_task(self.market.sell(self.price, transaction))
                            transaction["data"]["counter"] = 0
